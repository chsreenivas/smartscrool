      ...
      {videos.map((video, index) => (
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent">
            <p className="text-white text-xs font-medium line-clamp-1 mb-1">{video.title}</p>
          ...
          <div className="absolute top-2 left-2">
            <span className="px-2 py-0.5 rounded-full bg-black/50 text-white text-xs backdrop-blur-sm">{video.category}</span>
          </div>
        </motion.button>
      ))}
    </div>
